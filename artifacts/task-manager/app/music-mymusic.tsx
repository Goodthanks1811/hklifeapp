...
function MusicNoteDouble({ size = 20, color = RED }: { size?: number; color?: string }) {
  return (
        ...
        {TRACKS.map((name, i) => (
          ...
          >
            <View style={s.trackIcon}>
              <MusicNoteDouble size={20} color={RED} />
            </View>
            <View style={s.trackInfo}>
              <Text style={[s.trackName, i === NOW_PLAYING && s.trackNamePlaying]} numberOfLines={2}>
                {name}
              </Text>
            </View>
      ...
      <View style={[s.player, { paddingBottom: insets.bottom + 12 }]}>
        <View style={s.playerTrack}>
          <View style={s.playerArt}>
            <MusicNoteDouble size={18} color={RED} />
          </View>
          <View style={{ flex: 1 }}>
