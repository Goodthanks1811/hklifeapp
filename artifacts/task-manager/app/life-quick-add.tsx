...
const CATEGORIES: Category[] = [
  {
    label: "Life Admin",
    value: "\uD83D\uDCDD Life Admin",
    bg:  "rgba(100,110,130,0.20)",
    bgA: "rgba(100,110,130,0.38)",
    tx:  "#FFFFFF",
  },
  {
  ...
  },
  {
    ...
    bgA: "rgba(59,130,246,0.35)",
    tx:  "#FFFFFF",
  },
  {
    label: "Reference",
    value: "\uD83D\uDCCC Reference",
    bg:  "rgba(255,60,200,0.16)",
    bgA: "rgba(255,60,200,0.32)",
    tx:  "#FFFFFF",
  },
];
...
export default function LifeQuickAdd() {
  ...
  const handleSave = useCallback(async () => {
    ...
    const catObj = CATEGORIES.find((c) => c.label === selectedCat) ?? CATEGORIES[0];
  ...
  }, [title, schema, apiKey, selectedCat, selectedEmoji, shake, runLoader]);
  ...
  return (
    <View style={[styles.root, { paddingTop: topPad }]}>
      <ScreenHeader title="Life Quick Add" />
          ...
          <View style={{ alignSelf: "center", width: "100%", maxWidth: contentW }}>
            ...
            <View style={styles.formCard}>
              ...
              <View style={styles.catGrid}>
                {CATEGORIES.map(({ label, bg, bgA, tx }) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.catBtn,
                        { backgroundColor: isActive ? bgA : bg, opacity: isDimmed ? 0.18 : 1 },
                      ]}
                    >
                      <Text style={[styles.catBtnText, { color: tx }]}>{label}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
        ...
        <Animated.View style={[styles.loaderOverlay, { opacity: overlayOpacity }]} pointerEvents="auto">
          ...
          <Animated.View style={[styles.circleWrap, { opacity: circleOpacity, transform: [{ scale: circleScale }] }]}>
            <Animated.View style={{ transform: [{ scale: tickScale }] }}>
              <Feather name="check" size={40} color="#000" strokeWidth={3} />
            </Animated.View>
          </Animated.View>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  catBtnText: { fontSize: 11, fontWeight: "700", fontFamily: "Inter_700Bold", textAlign: "center" },
