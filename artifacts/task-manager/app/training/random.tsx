...
const STORAGE_KEY = "training_today_random";
    ...
    onPanResponderRelease: (_, gs) => {
      if (gs.dy > 80 || gs.vy > 0.8) {
        ...
        Animated.timing(slideAnim, {
          toValue: SCREEN_H, duration: 200, useNativeDriver: true, easing: Easing.in(Easing.quad),
      ...
      >
        ...
        <Animated.View style={{ transform: [{ translateY: slideAnim }] }}>
          <View style={sh.sheet}>
            ...
            >
              <Text style={sh.contextLabel}>{exercise?.bodyPart?.toUpperCase()} · EXERCISE NOTES</Text>
    ...
    <Animated.View style={[sc.rowOuter, { height: rowHeight, opacity: combinedOpacity }]}>
      ...
      >
        <Animated.View style={[sc.rowWrap, isDragging && sc.rowDragging, exercise.done && sc.rowWrapDone]}>
          ...
          >
            <Text style={[sc.rowTitle, exercise.done && sc.rowTitleDone]} numberOfLines={1}>
              {exercise.name}
            </Text>
          </Pressable>
...
// ── BaseRow — with body part label badge ──────────────────────────────────────
function BaseRow({
  exercise,
  onMoveToToday,
  onCrossDragStart,
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, gs) => { if (!isCrossDraggingRef.current) return; onCrossDragMove(gs.moveY); },
...
export default function RandomTrainingScreen() {
  ...
  // Group exercises by body part, filtering out already-added ones
  const groupedExercises = useMemo(() => {
    return ALL_BODY_PARTS.map(bp => ({
      bodyPart: bp,
      exercises: allExercises.filter(ex => ex.bodyPart === bp && !todayIds.has(ex.id)),
    })).filter(g => g.exercises.length > 0);
  }, [allExercises, todayIds]);

  const totalVisibleCount = useMemo(
    () => groupedExercises.reduce((sum, g) => sum + g.exercises.length, 0),
    [groupedExercises],
  );
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(raw => {
      if (raw) {
        try { setTodayList(JSON.parse(raw)); } catch {}
      }
    });
  }, []);

  useEffect(() => {
    AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(todayList));
  }, [todayList]);
  ...
  const fetchExercises = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const r    = await fetch(`${BASE_URL}/api/training/exercises/all`, {
  ...
  const animatePositions = useCallback((dragIdx: number, hoverIdx: number) => {
    ...
    cur.forEach((ex, i) => {
      ...
      Animated.timing(posAnims.current[ex.localId], {
        toValue: target * SLOT_H, duration: 110, useNativeDriver: true, easing: Easing.out(Easing.quad),
      }).start();
    });
  }, []);
  ...
  const handleStartRemove = useCallback((localId: string, _dur: number) => {
    ...
    todayListRef.current.forEach((ex, i) => {
      ...
      posAnims.current[ex.localId]?.stopAnimation();
      Animated.timing(posAnims.current[ex.localId], {
        toValue: target * SLOT_H, duration: 280, useNativeDriver: true, easing: Easing.out(Easing.quad),
      }).start();
    });
  }, []);
  ...
  return (
    <View style={sc.root}>
      ...
      <View style={[sc.gradHeader, { paddingTop: topPad + 8, paddingBottom: 36 }]}>
        ...
        <Text style={sc.gradTitle}>Random</Text>
      </View>
      ...
      {loading ? (
        ...
        >
          ...
          <View ref={allExSectionRef} style={sc.sectionHeader}>
            ...
            <Text style={sc.sectionCount}>{totalVisibleCount}</Text>
          </View>

          {totalVisibleCount === 0 && allExercises.length === 0 ? (
            ...
            <View style={{ paddingHorizontal: 16 }}>
              {groupedExercises.map(group => (
                <View key={group.bodyPart}>
                  <Text style={sc.bodyPartHeading}>{group.bodyPart.toUpperCase()}</Text>
                  <View style={{ gap: ITEM_GAP, marginBottom: 16 }}>
...
const sh = StyleSheet.create({
  ...
  handle: {
    ...
    alignSelf: "center", marginTop: 12, marginBottom: 16, flexShrink: 0,
  },
  contextLabel: {
    color: Colors.primary, fontSize: 10, fontFamily: "Inter_700Bold", letterSpacing: 1.4, marginBottom: 6,
  },
  exerciseTitle: {
    color: "#FFFFFF", fontSize: 24, fontFamily: "Inter_700Bold", lineHeight: 30, marginBottom: 10,
  },
  setupInput: {
  ...
  colHeaderRow: { flexDirection: "row", alignItems: "center", gap: 16, marginBottom: 4 },
  colHeader: {
  ...
  setLabel: { width: 60, color: "#FFFFFF", fontSize: 15, fontFamily: "Inter_700Bold", flexShrink: 0 },
  setInput: {
    width: 92,
    backgroundColor: "#1c1c1c", borderRadius: 10, borderWidth: 1, borderColor: Colors.border,
    paddingVertical: 12, paddingHorizontal: 4,
    color: "#FFFFFF", fontSize: 17, fontFamily: "Inter_700Bold", textAlign: "center",
  },
  ...
  notesLabel: {
    color: "#FFFFFF", fontSize: 10, fontFamily: "Inter_700Bold", letterSpacing: 1.2, marginBottom: 8,
  },
  notesInput: {
    color: Colors.textPrimary, fontSize: 15, fontFamily: "Inter_600SemiBold",
    paddingTop: 10, paddingBottom: 13, paddingHorizontal: 14,
  ...
  bottomRow: { flexDirection: "row", gap: 10, paddingTop: 12, flexShrink: 0 },
  addSetBtn: {
...
const sc = StyleSheet.create({
  ...
  bodyPartHeading: {
    ...
    letterSpacing: 1.4, marginTop: 8, marginBottom: 8,
  },
  ...
  progressTrack: {
    flex: 1, height: 4, backgroundColor: "#1e1e1e", borderRadius: 2, overflow: "hidden",
  },
